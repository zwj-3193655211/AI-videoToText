...
from PySide6.QtGui import QMouseEvent, QColor, QCursor, QFont
...
class TranslateSettingsDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        # 保存父窗口的原始设置
        self.original_opacity = parent.windowOpacity() if parent else self.settings_manager.get_opacity('translate')
        self.original_font_size = parent.subtitle_text.font().pointSize() if parent else self.settings_manager.get_font_size('translate')
...
class TranslateSubtitleWindow(QWidget):
    """翻译字幕窗口"""
    # 定义调整大小的区域大小
    RESIZE_MARGIN = 5
    ...
    def __init__(self):
        ...
        # 设置默认透明度
        self.setWindowOpacity(self.settings_manager.get_opacity('translate'))
        ...
        y = screen.height() - self.height() - 150  # 距离底部150像素，在原字幕窗口上方
        self.move(x, y)
    ...
    def _init_ui(self):
        """初始化UI"""
        # 加载UI文件
        ui_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "translate_subtitle.ui")
        loader = QUiLoader()
        ...
        self.subtitle_text.setText("翻译字幕将在这里显示")
        font = QFont()
        self.settings_manager = SettingsManager()
        font.setPointSize(self.settings_manager.get_font_size('translate'))
    ...
    def _show_settings(self):
        """显示设置对话框"""
        dialog = TranslateSettingsDialog(self)
